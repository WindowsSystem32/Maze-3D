...
boolean hm = false;
String txt;
int mm = 200; //이 이상으로 움직이면 도착했을 때 실패가 나타납니다.
...
void setup() {
  ...
  mod = 0;
  font = loadFont("NanumSquareRoundOTFR-128.vlw");
  iwid = img.width;
  ihei = img.height;
...
void draw() {
  ...
  if (!ep || !sp){
    ...
    if (mod == 0) {
      ...
      cmx = mouseX - half_wid;
      if (use_mouse) {
        deg = map(mouseX, 0, width, -360, 360);
      } else {
        deg = d;
      }
      rotateY(radians(deg));
      ...
      textSize(50);
      text("이동 횟수:", width / 2 - 300, height / 2 + 50);
      textSize(200);
